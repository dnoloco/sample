...
<?php if ($is_new_view): ?>
    ...
    <div class="mypco-builder">
        <!-- Left Panel -->
        <div class="mypco-builder-left">
            <h3><?php _e('Select a Module', 'mypco-online'); ?></h3>
            <select id="mypco-module-select">
                <option value=""><?php _e('Choose...', 'mypco-online'); ?></option>
                <?php foreach ($modules as $mod_key => $mod_name): ?>
                    <option value="<?php echo esc_attr($mod_key); ?>"><?php echo esc_html($mod_name); ?></option>
                <?php endforeach; ?>
            </select>

            <h3><?php _e('Shortcode List', 'mypco-online'); ?></h3>
        ...
        <!-- Right Panel -->
        <div class="mypco-builder-right">
            <div id="mypco-builder-placeholder">
                <p><?php _e('Select a shortcode type to configure.', 'mypco-online'); ?></p>
            </div>

            <form method="post" action="" id="mypco-builder-form" style="display:none;">
                ...
                <input type="hidden" name="shortcode_type" id="mypco-builder-type" value="">

                <h3><?php _e('Shortcode Settings', 'mypco-online'); ?></h3>
                ...
                <?php foreach ($types as $slug => $type_def): ?>
                    <?php if (!empty($type_def['fields'])): ?>
                        <div class="mypco-type-fields" data-type="<?php echo esc_attr($slug); ?>">
                ...
                <h4><?php _e('Styling', 'mypco-online'); ?></h4>
                ...
                <div class="mypco-field">
                    ...
                    <input type="text" id="custom_class" name="custom_class" class="regular-text"
                           placeholder="<?php esc_attr_e('my-custom-class', 'mypco-online'); ?>">
                </div>

                <div class="mypco-field-row">
                    <div class="mypco-field mypco-field-color">
                        <label for="primary_color"><?php _e('Primary', 'mypco-online'); ?></label>
                        <input type="color" id="primary_color" name="primary_color" value="#333333">
                    </div>
                    <div class="mypco-field mypco-field-color">
                        <label for="text_color"><?php _e('Text', 'mypco-online'); ?></label>
                        <input type="color" id="text_color" name="text_color" value="#333333">
                    </div>
                    <div class="mypco-field mypco-field-color">
                        <label for="background_color"><?php _e('Background', 'mypco-online'); ?></label>
                        <input type="color" id="background_color" name="background_color" value="#ffffff">
                    </div>
                    <div class="mypco-field mypco-field-color">
                        <label for="border_radius"><?php _e('Radius', 'mypco-online'); ?></label>
                        <input type="number" id="border_radius" name="border_radius" value="8" min="0" max="30" class="small-text"> px
                    </div>
                </div>

                <p class="submit">
                    <button type="submit" class="button button-primary"><?php _e('Create Shortcode', 'mypco-online'); ?></button>
    ...
    (function($) {
        ...
        $('#mypco-module-select').on('change', function() {
            var mod = $(this).val();
            $('.mypco-type-list').hide();
            $('.mypco-type-link').removeClass('active');
            $('#mypco-builder-form').hide();
            $('#mypco-builder-placeholder').show();

            if (mod) {
                $('#mypco-type-hint').hide();
                $('.mypco-type-list[data-module="' + mod + '"]').show();
            } else {
                $('#mypco-type-hint').show();
            }
        });

        // Type link click → show the settings form
        $('.mypco-type-link').on('click', function(e) {
            e.preventDefault();
...
<?php elseif ($is_edit_view): ?>
    <?php // ================================================================
          ...
          //   $type_slug (string) — e.g. 'mypco_calendar'
    ...
    <form method="post" action="">
        ...
        <div class="card">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="shortcode_description"><?php _e('Description', 'mypco-online'); ?></label></th>
                    <td>
        ...
        <div class="card">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="custom_class"><?php _e('Custom CSS Class', 'mypco-online'); ?></label></th>
                ...
                <tr>
                    <th scope="row"><label for="primary_color"><?php _e('Primary Color', 'mypco-online'); ?></label></th>
                    <td><input type="color" id="primary_color" name="primary_color" value="<?php echo esc_attr($shortcode['primary_color'] ?? '#333333'); ?>"> <span class="mypco-color-preview"><?php echo esc_html($shortcode['primary_color'] ?? '#333333'); ?></span></td>
                </tr>
                <tr>
                    <th scope="row"><label for="text_color"><?php _e('Text Color', 'mypco-online'); ?></label></th>
                    <td><input type="color" id="text_color" name="text_color" value="<?php echo esc_attr($shortcode['text_color'] ?? '#333333'); ?>"> <span class="mypco-color-preview"><?php echo esc_html($shortcode['text_color'] ?? '#333333'); ?></span></td>
                </tr>
                <tr>
                    <th scope="row"><label for="background_color"><?php _e('Background Color', 'mypco-online'); ?></label></th>
                    <td><input type="color" id="background_color" name="background_color" value="<?php echo esc_attr($shortcode['background_color'] ?? '#ffffff'); ?>"> <span class="mypco-color-preview"><?php echo esc_html($shortcode['background_color'] ?? '#ffffff'); ?></span></td>
                </tr>
                <tr>
                    <th scope="row"><label for="border_radius"><?php _e('Border Radius', 'mypco-online'); ?></label></th>
        ...
        <p class="submit">
            <button type="submit" class="button button-primary"><?php _e('Save Settings', 'mypco-online'); ?></button>
            <a href="<?php echo esc_url($page_url); ?>" class="button"><?php _e('Cancel', 'mypco-online'); ?></a>
        </p>
    </form>
        ...
        $('.mypco-copy-btn').on('click', function() {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(text).then(function() {
                    ...
                    setTimeout(function() { $btn.text('<?php echo esc_js(__('Copy', 'mypco-online')); ?>'); }, 2000);
                });
            }
        });
...
<?php else: ?>
    ...
    <?php if ($settings_saved): ?>
        <div class="notice notice-success is-dismissible"><p><?php _e('Shortcode settings saved successfully!', 'mypco-online'); ?></p></div>
    <?php endif; ?>
    <?php if ($deleted): ?>
        <div class="notice notice-success is-dismissible"><p><?php _e('Shortcode deleted.', 'mypco-online'); ?></p></div>
    <?php endif; ?>
    <?php if (!empty($bulk_deleted)): ?>
        <div class="notice notice-success is-dismissible"><p><?php printf(_n('%d shortcode deleted.', '%d shortcodes deleted.', $bulk_deleted, 'mypco-online'), $bulk_deleted); ?></p></div>
    <?php endif; ?>
    ...
    <ul class="subsubsub">
        <li class="all">
            <a href="<?php echo esc_url($page_url); ?>" <?php echo empty($current_filter) ? 'class="current" aria-current="page"' : ''; ?>>
                <?php _e('All', 'mypco-online'); ?> <span class="count">(<?php echo esc_html($count_all); ?>)</span>
            </a>
        </li>
        <?php foreach ($modules as $mod_key => $mod_name): ?>
                    ...
                    <tr id="shortcode-<?php echo esc_attr($sc_id); ?>">
                        ...
                        <td class="shortcode column-shortcode has-row-actions column-primary">
                            ...
                            <div class="row-actions">
                                ...
                                | <span class="copy"><a href="#" class="mypco-copy-link" data-copy="<?php echo esc_attr($sc_code); ?>"><?php _e('Copy', 'mypco-online'); ?></a></span>
                                | <span class="trash"><a href="<?php echo $trash_url; ?>" class="submitdelete" onclick="return confirm('<?php echo esc_js(__('Are you sure you want to delete this shortcode?', 'mypco-online')); ?>');"><?php _e('Trash', 'mypco-online'); ?></a></span>
                            </div>
                        </td>
                        <td class="description column-description"><?php echo !empty($sc_desc) ? esc_html($sc_desc) : '<span class="mypco-no-description">&mdash;</span>'; ?></td>
    ...
    (function($) {
        ...
        $('.mypco-copy-link').on('click', function(e) {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(text).then(function() {
                    $l.text('<?php echo esc_js(__('Copied!', 'mypco-online')); ?>');

 ...
 *
 * Upload this to your Hostinger site: your-site.com/mypco-license/api.php
 */
...
function is_valid_key_format($key) {
    // Format: MYPCO-XXXX-XXXX-XXXX-XXXX (alphanumeric)
    return preg_match('/^MYPCO-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/', $key);
}
...
function get_license($license_key) {
    ...
    $stmt = $pdo->prepare('
        SELECT l.*,
               ...
               (SELECT GROUP_CONCAT(a2.site_url) FROM mypco_activations a2 WHERE a2.license_id = l.id) as activated_urls
        FROM mypco_licenses l
        LEFT JOIN mypco_activations a ON l.id = a.license_id
        WHERE l.license_key = ?
        GROUP BY l.id
    ');
...
function action_activate() {
    ...
    $pdo = get_db_connection();
    $stmt = $pdo->prepare('
        INSERT INTO mypco_activations (license_id, site_url, site_name, activated_at, last_check)
        VALUES (?, ?, ?, NOW(), NOW())
    ');
        ...
        if ($license && $license['status'] === 'active') {
            ...
            if (!$license['expires_at'] || strtotime($license['expires_at']) >= time()) {
                ...
                if (!empty($site_url)) {
                    ...
                    if ($activation) {
                        $pdo = get_db_connection();
                        $stmt = $pdo->prepare('UPDATE mypco_activations SET last_check = NOW() WHERE id = ?');
...
function action_generate() {
    ...
    $license_key = 'MYPCO-' . strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 4)) . '-'
    ...
    $pdo = get_db_connection();
    $stmt = $pdo->prepare('
        INSERT INTO mypco_licenses (license_key, customer_email, customer_name, tier, max_activations, status, expires_at, created_at)
        VALUES (?, ?, ?, ?, ?, "active", ?, NOW())
    ');

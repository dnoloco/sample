...
$back_url = remove_query_arg('mypco_message');
?>

<div class="mypco-message-single">

    <!-- Back link -->
    <a href="<?php echo esc_url($back_url); ?>" class="mypco-message-back">&larr; <?php _e('All Messages', 'mypco-online'); ?></a>

    <!-- Message title -->
    <h2 class="mypco-message-single-title"><?php echo esc_html($message->title); ?></h2>

    <!-- Video (large, full-width) -->
    <?php if ($has_video): ?>
        <div class="mypco-message-single-video"
             data-embed-url="<?php echo esc_attr($video['embed_url']); ?>"
             data-video-type="<?php echo esc_attr($video['type']); ?>">
            <?php if ($video['type'] === 'direct'): ?>
                <video class="mypco-message-single-video-element" controls preload="none"
    ...
    <?php if (!empty($message->audio_url)): ?>
        <div class="mypco-message-single-audio">
    ...
    <!-- Message info -->
    <div class="mypco-message-single-info">

        <?php if (!empty($message->description)): ?>
            <div class="mypco-message-single-description">
                <?php echo wpautop(esc_html($message->description)); ?>
            </div>
        <?php endif; ?>

        <div class="mypco-message-single-meta">
            <?php if (!empty($message->speaker_name)): ?>
                <div class="mypco-message-single-meta-row">
                    <span class="mypco-message-single-label"><?php _e('Speaker', 'mypco-online'); ?></span>
                    <span class="mypco-message-single-value"><?php echo esc_html($message->speaker_name); ?></span>
                </div>
            <?php endif; ?>

            <?php if (!empty($message_date)): ?>
                <div class="mypco-message-single-meta-row">
                    <span class="mypco-message-single-label"><?php _e('Date', 'mypco-online'); ?></span>
                    <span class="mypco-message-single-value"><?php echo esc_html($message_date); ?></span>
                </div>
            <?php endif; ?>

            <?php if (!empty($message->series_title)): ?>
                <div class="mypco-message-single-meta-row">
                    <span class="mypco-message-single-label"><?php _e('Series', 'mypco-online'); ?></span>
                    <span class="mypco-message-single-value"><?php echo esc_html($message->series_title); ?></span>
                </div>
            <?php endif; ?>

            <?php if (!empty($message->scripture)): ?>
                <div class="mypco-message-single-meta-row">
                    <span class="mypco-message-single-label"><?php _e('Scripture', 'mypco-online'); ?></span>
                    <span class="mypco-message-single-value"><?php echo esc_html($message->scripture); ?></span>
                </div>
            <?php endif; ?>

            <?php if (!empty($message->topic_name)): ?>
                <div class="mypco-message-single-meta-row">
                    <span class="mypco-message-single-label"><?php _e('Topic', 'mypco-online'); ?></span>
                    <span class="mypco-message-single-value"><?php echo esc_html($message->topic_name); ?></span>
                </div>
            <?php endif; ?>
        </div>
    </div>

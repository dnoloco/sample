...
class MyPCO_Services_Admin
{
    ...
    public function enqueue_admin_assets($hook)
    {
        ...
        if (strpos($hook, 'mypco-services') === false) {
            return;
        }

        wp_enqueue_style(
            'mypco-services-admin',
            MYPCO_PLUGIN_URL . 'modules/services/admin/assets/css/services-admin.css',
            [],
            MYPCO_VERSION
        );
        ...
        wp_enqueue_script(
            'mypco-services-admin',
            MYPCO_PLUGIN_URL . 'modules/services/admin/assets/js/services-admin.js',
            ['jquery'],
            MYPCO_VERSION,
            true
        );
    ...
    private function get_success_message()
    {
        ...
        if ($_GET['message'] === 'sent' || $_GET['message'] === 'scheduled') {
            $success_data = get_transient('mypco_clearstream_success_message');
            if ($success_data) {
                delete_transient('mypco_clearstream_success_message');
                return $success_data;
            }
        }
    ...
    private function get_error_message()
    {
        ...
        $error = get_transient('mypco_clearstream_error_message');
        if ($error) {
            delete_transient('mypco_clearstream_error_message');
            return $error;
        }
    ...
    public function handle_clearstream_send()
    {
        ...
        if (!$this->user_can_send_clearstream()) {
            wp_redirect(admin_url('admin.php?page=mypco-services&message=no_permission'));
            exit;
        }
        ...
        if (empty($message_body)) {
            set_transient('mypco_clearstream_error_message', [
                'code' => 400,
                'message' => 'Message body is required'
            ], 60);
            wp_redirect(admin_url('admin.php?page=mypco-services&view=clearstream_compose&message=error'));
            exit;
        }
        ...
        if ($result['success']) {
            set_transient('mypco_clearstream_success_message', [
                'count' => $result['count'],
                'status' => $send_type,
            ...
            wp_redirect(admin_url('admin.php?page=mypco-services&message=' . $message_param));
        } else {
            set_transient('mypco_clearstream_error_message', [
                'code' => $result['code'],

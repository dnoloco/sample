...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentView = 'mypco_*-view-list';
    let previousView = 'mypco-view-list';
    ...
    const viewButtons = document.querySelectorAll('.mypco-view-btn');
    const viewSections = document.querySelectorAll('.mypco-view-section');
    ...
    function switchView(viewId) {
        if (currentView !== 'mypco-view-detail') {
        ...
        document.body.classList.remove('mypco-view-list-active', 'mypco-view-month-active', 'mypco-view-gallery-active', 'mypco-view-detail-active');
        document.body.classList.add(viewId + '-active');

        currentView = viewId;

        if (viewId === 'mypco-view-month') {
            renderMonthView();
        }
    }
    ...
    const calendarGrid = document.querySelector('.mypco-mini-cal-grid');
    const monthDisplay = document.querySelector('.mypco-mini-cal-month-display');
    const navButtons = document.querySelectorAll('.mypco-mini-cal-nav');
    ...
    function collectEventDates() {
        ...
        document.querySelectorAll('.mypco-event-date[data-date], .mypco-day-header[data-date]').forEach(el => {
    ...
    function renderMiniCalendar(month, year) {
        ...
        dayNames.forEach(day => {
            ...
            header.className = 'mypco-day-header-cell';
        ...
        for (let i = 0; i < firstDayOfMonth; i++) {
            ...
            empty.className = 'mypco-empty-cell';
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            dateCell.className = 'mypco-calendar-date';
    ...
    function scrollToDate(dateStr) {
        switchView('mypco-view-list');
        setTimeout(() => {
            const targetSection = document.querySelector(`.mypco-day-header[data-date="${dateStr}"], .mypco-event-date[data-date="${dateStr}"]`);
            if (targetSection) {
                targetSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                targetSection.classList.add('mypco-highlight');
                setTimeout(() => targetSection.classList.remove('mypco-highlight'), 2000);
            }
        }, 100);
    }
    ...
    function initListViewPagination() {
        const eventItems = document.querySelectorAll('#mypco-view-list .mypco-event-item');
        const dayHeaders = document.querySelectorAll('#mypco-view-list .mypco-day-header');
        const monthHeaders = document.querySelectorAll('#mypco-view-list .mypco-month-header');

        if (eventItems.length === 0) return;

        // Create load more button if it doesn't exist
        let loadMoreBtn = document.getElementById('mypco-load-more-btn');
        if (!loadMoreBtn) {
            loadMoreBtn = document.createElement('button');
            loadMoreBtn.id = 'mypco-load-more-btn';
            loadMoreBtn.className = 'mypco-load-more-btn';
            loadMoreBtn.textContent = 'Load More Events';

            // Insert after the last event item
            const listView = document.getElementById('mypco-view-list');
            if (listView) {
        ...
        function updateVisibility() {
            ...
            eventItems.forEach((item, index) => {
                if (index < visibleEventCount) {
                    ...
                    while (prevSibling) {
                        if (prevSibling.classList.contains('mypco-day-header')) {
                            visibleDates.add(prevSibling.getAttribute('data-date'));
                            break;
                        }
                        if (prevSibling.classList.contains('mypco-month-header')) {
            ...
            dayHeaders.forEach(header => {
                ...
                while (nextSibling && !nextSibling.classList.contains('mypco-day-header') && !nextSibling.classList.contains('mypco-month-header')) {
                    if (nextSibling.classList.contains('mypco-event-item') && nextSibling.style.display !== 'none') {
            ...
            monthHeaders.forEach(header => {
                ...
                let nextSibling = header.nextElementSibling;
                while (nextSibling && !nextSibling.classList.contains('mypco-month-header')) {
                    if (nextSibling.classList.contains('mypco-day-header') && nextSibling.style.display !== 'none') {
                        hasVisibleDay = true;
                        break;
                    }
                    nextSibling = nextSibling.nextElementSibling;
    ...
    function renderMonthView() {
        const monthContainer = document.querySelector('.mypco-month-calendar-container');
        ...
        let html = `
            <div class="mypco-month-view-header">
                <h2 class="mypco-month-view-title">${monthNames[currentMonth]} ${currentYear}</h2>
                <div class="mypco-month-view-nav">
                    <button class="mypco-month-nav-btn" data-dir="prev">< Previous</button>
                    <button class="mypco-month-nav-today">Today</button>
                    <button class="mypco-month-nav-btn" data-dir="next">Next ></button>
                </div>
            </div>
            <div class="mypco-month-calendar-grid">
                <div class="mypco-month-day-header">Sun</div>
                <div class="mypco-month-day-header">Mon</div>
                <div class="mypco-month-day-header">Tue</div>
                <div class="mypco-month-day-header">Wed</div>
                <div class="mypco-month-day-header">Thu</div>
                <div class="mypco-month-day-header">Fri</div>
                <div class="mypco-month-day-header">Sat</div>
        `;

        // Empty cells before first day
        for (let i = 0; i < firstDay; i++) {
            html += '<div class="mypco-month-day-cell mypco-month-day-empty"></div>';
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            let cellClass = 'mypco-month-day-cell';
            if (isToday) cellClass += ' mypco-month-day-today';

            // Get events for this date from pcoExpandedEvents
            const dayEvents = eventsByDate[dateStr] || [];

            html += `<div class="${cellClass}">
                <div class="mypco-month-day-number">${day}</div>
                <div class="mypco-month-day-events">`;
            ...
            dayEvents.slice(0, maxEventsToShow).forEach((event) => {
                ...
                html += `<div class="mypco-month-event" data-event='${JSON.stringify(event).replace(/'/g, "&#39;")}'>${displayTime}${eventName}</div>`;
            });

            // Show "+X more" if there are additional events
            if (dayEvents.length > maxEventsToShow) {
                html += `<div class="mypco-month-event-more">+${dayEvents.length - maxEventsToShow} more</div>`;
            }
        ...
        monthContainer.querySelectorAll('.mypco-month-event').forEach(el => {
        ...
        // Navigation handlers
        monthContainer.querySelectorAll('.mypco-month-nav-btn').forEach(btn => {
            btn.addEventListener('click', function() {
        ...
        monthContainer.querySelector('.mypco-month-nav-today')?.addEventListener('click', function() {
            const now = new Date();
    ...
    function showEventDetail(eventData, eventIndex) {
        ...
        const detailView = document.getElementById('mypco-view-detail');
        if (!detailView) return;

        document.getElementById('mypco-detail-title').textContent = eventData.name || '';
        document.getElementById('mypco-detail-date').textContent = eventData.date || '';
        document.getElementById('mypco-detail-time').textContent = eventData.time || '';
        document.getElementById('mypco-detail-description').innerHTML = formatDescription(eventData.description || eventData.summary || '');

        // Image
        const detailImage = document.getElementById('mypco-detail-image');
        const imageContainer = document.getElementById('mypco-detail-image-container');
        ...
        const locationContainer = document.getElementById('mypco-detail-location-container');
        const locationLink = document.getElementById('mypco-detail-location-link');
        const mapContainer = document.getElementById('mypco-detail-map-container');
        const mapIframe = document.getElementById('mypco-detail-map-iframe');
        ...
        if (eventData.location) {
            ...
            document.getElementById('mypco-detail-location-text').textContent = eventData.location_name || eventData.location;
            document.getElementById('mypco-detail-address').textContent = eventData.location;
            ...
            document.getElementById('mypco-detail-location-text').textContent = '';
            document.getElementById('mypco-detail-address').textContent = '';
        ...
        // Signup button
        const signupBtn = document.getElementById('mypco-detail-signup-btn');
        const signupContainer = document.getElementById('mypco-detail-signup-container');
        ...
        // Breadcrumb
        document.getElementById('mypco-breadcrumb-event-name').textContent = eventData.name || '';

        // Update navigation
        updateDetailNavigation();

        // Switch to detail view
        switchView('mypco-view-detail');
    ...
    function updateDetailNavigation() {
        const prevBtn = document.getElementById('mypco-detail-prev');
        const nextBtn = document.getElementById('mypco-detail-next');
    ...
    document.querySelectorAll('.mypco-event-title-btn').forEach(function(btn) {
    ...
    const backBtn = document.getElementById('mypco-detail-back');
    if (backBtn) {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('mypco-signup-link')) {
            e.preventDefault();
            e.stopPropagation();

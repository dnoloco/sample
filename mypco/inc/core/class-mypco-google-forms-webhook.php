<?php
// includes/class-mypco-google-forms-webhook.php

class MyPCO_Google_Forms_Webhook {

    private $signups_handler;
    private $stripe_handler;

    public function __construct() {
        // Hook registration handled by the centralized loader in mypco.php
    }
    ...
    public function register_rest_route() {
        register_rest_route('mypco-forms/v1', '/submit', [
            'methods' => 'POST',
    ...
    public function verify_webhook_secret($request) {
        ...
        $stored_secret = get_option('mypco_google_forms_secret');
    ...
    private function process_signup_registration($signup, $responses, $email, $timestamp, $response_id) {
        ...
        do_action('mypco_signup_registration_received', $registration_id, $signup->id, $registration_data);
    ...
    private function log_generic_submission($form_id, $form_title, $response_id, $email, $timestamp, $responses) {
        global $wpdb;
        $table_name = $wpdb->prefix . 'mypco_form_submissions';
    ...
    public function get_submission($id) {
        global $wpdb;
        $table = $wpdb->prefix . 'mypco_form_submissions';

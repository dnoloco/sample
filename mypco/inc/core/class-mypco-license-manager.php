...
class MyPCO_License_Manager {
    ...
    private function __construct() {
        ...
        $this->license_data = get_transient('mypco_license_data');
    }
    ...
    public function get_license_key() {
        return get_option('mypco_license_key', null);
    }
    ...
    public function get_license_data($force_refresh = false) {
        ...
        if ($response && $response['success']) {
            ...
            set_transient('mypco_license_data', $this->license_data, $this->cache_duration);

            // Update legacy option for backwards compatibility
            update_option('mypco_license_status', 'active');
        } else {
            $this->license_data = null;
            delete_transient('mypco_license_data');
            update_option('mypco_license_status', 'inactive');
        }
    ...
    public function activate_license($license_key) {
        ...
        if ($response && $response['success']) {
            ...
            update_option('mypco_license_key', $license_key);
            update_option('mypco_license_status', 'active');
    ...
    public function deactivate_license() {
        ...
        delete_option('mypco_license_key');
        update_option('mypco_license_status', 'inactive');
        delete_transient('mypco_license_data');
        $this->license_data = null;
    ...
    private function api_request($action, $data = []) {
        ...
        if (is_wp_error($response)) {
            error_log('MyPCO License API Error: ' . $response->get_error_message());
            return null;
        }

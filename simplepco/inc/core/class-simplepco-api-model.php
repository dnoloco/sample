...
class SimplePCO_API_Model {
     ...
     *
     * Tries the /v2/episodes/{id} endpoint to get the full file data,
     * then falls back to constructing a URL from the signed_identifier.
     *
     * @param string $episode_id  The PCO episode ID.
     * @param string $signed_id   The signed_identifier from the sermon_audio File.
     * @param string $filename    The filename from the sermon_audio File.
     * @return string|false The audio URL or false on failure.
     */
    public function get_sermon_audio_url($episode_id, $signed_id = '', $filename = '') {
        // Try fetching the episode with full file data
        $url = $this->base_url . "/publishing/v2/episodes/{$episode_id}";
        $response = wp_remote_get($url, [
            'timeout' => 15,
            'headers' => [
                'Authorization' => $this->auth_header,
        ...
        if (!is_wp_error($response)) {
            ...
            $sa = $body['data']['attributes']['sermon_audio'] ?? [];
            $sa_attrs = $sa['attributes'] ?? [];

            // Check if the full response has more data than the cached version
            if (!empty($sa_attrs['url'])) {
                error_log('[SimplePCO] Sermon audio URL from API for episode ' . $episode_id . ': ' . $sa_attrs['url']);
                return $sa_attrs['url'];
            }
            if (!empty($sa_attrs['variants'])) {
                $variants = $sa_attrs['variants'];
                if (is_array($variants)) {
                    $resolved = $variants['original'] ?? reset($variants);
                    error_log('[SimplePCO] Sermon audio variant URL for episode ' . $episode_id . ': ' . $resolved);
                    return $resolved;
                }
            }
            if (!empty($sa_attrs['download_url'])) {
                error_log('[SimplePCO] Sermon audio download_url for episode ' . $episode_id . ': ' . $sa_attrs['download_url']);
                return $sa_attrs['download_url'];
            }
            if (!empty($sa_attrs['streaming_url'])) {
                error_log('[SimplePCO] Sermon audio streaming_url for episode ' . $episode_id . ': ' . $sa_attrs['streaming_url']);
                return $sa_attrs['streaming_url'];
            }

            // Log all sermon_audio attributes for debugging
            error_log('[SimplePCO] Sermon audio full attributes for episode ' . $episode_id . ': ' . wp_json_encode($sa));

            // Check for links on the sermon_audio object
            $sa_links = $sa['links'] ?? [];
            if (!empty($sa_links)) {
                error_log('[SimplePCO] Sermon audio links for episode ' . $episode_id . ': ' . wp_json_encode($sa_links));
                if (!empty($sa_links['streaming'])) return $sa_links['streaming'];
                if (!empty($sa_links['download'])) return $sa_links['download'];
                if (!empty($sa_links['self'])) return $sa_links['self'];
            }
        } else {
            error_log('[SimplePCO] Episode fetch error for ' . $episode_id . ': ' . $response->get_error_message());
        }

        error_log('[SimplePCO] Could not resolve sermon audio URL for episode ' . $episode_id);
        return false;
    }

...
class SimplePCO_Calendar_Public {
    ...
    public function init() {
        // Register shortcodes
        add_shortcode('simplepco_calendar', [$this, 'render_calendar_shortcode']);
        add_shortcode('pco_calendar', [$this, 'render_calendar_shortcode']); // Backward compat

        // Register standalone view shortcodes
        add_shortcode('simplepco_calendar_list', [$this, 'render_calendar_list_shortcode']);
        add_shortcode('simplepco_calendar_month', [$this, 'render_calendar_month_shortcode']);
        add_shortcode('simplepco_calendar_gallery', [$this, 'render_calendar_gallery_shortcode']);
    ...
    public function enqueue_public_assets() {
        ...
        $has_calendar = has_shortcode($post->post_content, 'simplepco_calendar') ||
                        has_shortcode($post->post_content, 'pco_calendar') ||
                        has_shortcode($post->post_content, 'simplepco_calendar_list') ||
                        has_shortcode($post->post_content, 'simplepco_calendar_month') ||
                        has_shortcode($post->post_content, 'simplepco_calendar_gallery');
        ...
        ';
        wp_add_inline_style('simplepco-calendar-public', $critical_css);
    ...
    public function render_calendar_shortcode($atts) {
        ...
        if ($id > 0) {
            ...
            $settings = SimplePCO_Shortcodes_Admin::get_shortcode_settings($id, 'simplepco_calendar_list');
        } else {
            $settings = [];
        }
        ...
        wp_localize_script('simplepco-calendar-public', 'simplepcoCalendarData', [
    ...
    public function render_calendar_list_shortcode($atts) {
        return $this->render_single_view_shortcode('list', $atts, 'simplepco_calendar_list');
    }
    ...
    public function render_calendar_month_shortcode($atts) {
        return $this->render_single_view_shortcode('month', $atts, 'simplepco_calendar_month');
    }
    ...
    public function render_calendar_gallery_shortcode($atts) {
        return $this->render_single_view_shortcode('gallery', $atts, 'simplepco_calendar_gallery');
    }
    ...
    private function render_single_view_shortcode($view, $atts, $tag) {
        ...
        if ($id > 0) {
            require_once SIMPLEPCO_PLUGIN_DIR . 'inc/core/class-simplepco-shortcodes-admin.php';
            $settings = SimplePCO_Shortcodes_Admin::get_shortcode_settings($id, 'simplepco_calendar_' . $view);
        } else {
            $settings = [];
        }
    ...
    private function fetch_calendar_data($atts) {
        ...
        $transient_key = 'simplepco_calendar_v2_' . md5(serialize($params));
    ...
    private function fetch_tags() {
        ...
        $transient_key = 'simplepco_calendar_tags_' . md5(serialize($params));
    ...
    private function format_event_instance($instance, $parent, $tag_ids = []) {
        ...
        try {
            $start_dt = SimplePCO_Date_Helper::parse_event_date($starts_at, $is_all_day, $this->timezone, false);
            $date_display = SimplePCO_Date_Helper::get_date_display($starts_at, $ends_at, $is_all_day, $this->timezone);
            $time_display = SimplePCO_Date_Helper::get_time_display($starts_at, $is_all_day, $this->timezone, $ends_at);
            $time_short = SimplePCO_Date_Helper::get_time_display($starts_at, $is_all_day, $this->timezone); // Start time only
    ...
    private function build_expanded_events($regular_events, $featured_events) {
        ...
        foreach ($all_events as $event) {
            ...
            if ($ends_at) {
                $event_dates = SimplePCO_Date_Helper::expand_multi_day_event($starts_at, $ends_at, $is_all_day, $this->timezone);
            } else {
    ...
    private function render_error($error_message) {
        return '<div class="simplepco-calendar-error"><p>' . esc_html($error_message) . '</p></div>';
    }
    ...
    private function load_template($template_name, $data = []) {
        ...
        $template_path = SIMPLEPCO_PLUGIN_DIR . 'templates/calendar/public/' . $template_name . '.php';

...
class SimplePCO_Signups_Public {
    ...
    public function __construct($loader, $api_model) {
        ...
        // Initialize handlers
        global $simplepco_webhook_handler, $simplepco_stripe_handler;
        if ($simplepco_webhook_handler) {
            $this->webhook_handler = $simplepco_webhook_handler;
        }
        if ($simplepco_stripe_handler) {
            $this->stripe_handler = $simplepco_stripe_handler;
        }
    }
    ...
    public function init() {
        ...
        $this->loader->add_action('wp_ajax_simplepco_process_payment', $this, 'ajax_process_payment');
        $this->loader->add_action('wp_ajax_nopriv_simplepco_process_payment', $this, 'ajax_process_payment');
    }
    ...
    public function register_webhook_routes() {
        register_rest_route('simplepco/v1', '/webhook/google-forms', [
            'methods' => 'POST',
    ...
    public function register_stripe_webhook() {
        register_rest_route('simplepco/v1', '/webhook/stripe', [
            'methods' => 'POST',
    ...
    public function verify_webhook_signature($request) {
        ...
        $secret = get_option('simplepco_webhook_secret');
    ...
    public function register_payment_shortcode() {
        add_shortcode('simplepco_payment_form', [$this, 'render_payment_form']);
    }
    ...
    public function render_payment_form($atts) {
        ...
        $registration = $simplepco_signups_handler->get_registration($registration_id);
        
        if (!$registration) {
            return $this->render_error('Registration not found.');
        }

        $signup = $simplepco_signups_handler->get_signup($registration->signup_id);
        ...
        $stripe_public_key = get_option('simplepco_stripe_public_key');
    ...
    public function ajax_process_payment() {
        ...
        if ($result['status'] === 'succeeded') {
            // Update registration
            $simplepco_signups_handler->update_payment_status($registration_id, [
                'payment_status' => 'paid',
    ...
    private function load_template($template_name, $data = []) {
        extract($data);
        $template_path = SIMPLEPCO_PLUGIN_DIR . 'templates/signups/public/' . $template_name . '.php';

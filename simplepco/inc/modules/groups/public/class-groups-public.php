...
class SimplePCO_Groups_Public {
    ...
    public function register_shortcodes() {
        ...
        add_shortcode('simplepco_groups', [$this, 'render_groups_shortcode']);
    }
    ...
    public function render_groups_shortcode($atts) {
        ...
        if ($id > 0) {
            ...
            $settings = SimplePCO_Shortcodes_Admin::get_shortcode_settings($id, 'simplepco_groups');
        } else {
            $settings = [];
        }
        ...
        if (isset($groups_data['error'])) {
            return '<div class="simplepco-groups-error"><p>' . esc_html($groups_data['error']) . '</p></div>';
        }
    ...
    private function load_template($template_name, $data = []) {
        extract($data);
        $template_path = SIMPLEPCO_PLUGIN_DIR . 'templates/groups/public/' . $template_name . '.php';
